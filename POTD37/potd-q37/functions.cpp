...
bool validUsername(std::string inputUser) {
  ...
  vecCopy.pop_back();
  if (vecCopy.size() == 0) {
    std::cout << "Your username was too short. Try again, ol' buddy ol' pal..."
              << std::endl;
    return false;
  }
...
int arrayIndex(int idx) {
  ...
  if (idx >= len) {
    std::cout
        << "This index is out of bounds. Please provide a valid array index..."
        << std::endl;
    exit(1);
  } else {
    return myArray[idx];
...
int factorial(int fact) {
  if (fact < 0 || fact > 12) {
    std::cout << "Please enter a number between 0 and 12 (inclusive)."
              << std::endl;
    exit(1);
  }
...
bool samesies(int one, int two) { return (one == two); }

int absolutely(int fact) { return std::abs(fact); }

int magnaCalca(int first, int second) {
  if (std::abs(first) > 100 || std::abs(second) > 100) {
    std::cout << "Please insert integers between -100 and 100 (inclusive)."
              << std::endl;
    exit(1);
  }
