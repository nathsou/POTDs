...
bool BTreeNode::rotateRight(unsigned idx, unsigned order) {
  ...
  /**
   * Then check if the left sibling leaf contains enough elements after one
   * being borrowed.
   */
  ...
  int left = prev->elements_.back();
  prev->children_.pop_back();
  int child = elements_[idx - 1];
  elements_[idx - 1] = left;
  children_[idx]->elements_.insert(children_[idx]->elements_.begin(), child);
